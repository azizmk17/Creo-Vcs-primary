...
from PyQt5.QtCore import Qt, QDateTime, pyqtSignal, QTimer, QObject, QThread, QSize, QPoint
from PyQt5.QtGui import QColor, QPen, QFont, QBrush, QPolygon
...
class BomPage(QWidget):
    ...
    def __init__(self, bom_service):
        ...
        self._cached_tree_items = []  # kept for legacy compat; no longer populated
        self._in_search_mode = False  # True while search results are displayed
        self.init_ui()
    ...
    def _set_tree_loading(self, loading: bool) -> None:
        try:
            ...
            elif getattr(self, "_in_search_mode", False):
                stack.setCurrentIndex(2)  # search results tree
            else:
                stack.setCurrentIndex(1)  # full tree (never cleared)
        except Exception:
            pass
    ...
    def _on_initial_diag_finished(self, missing_files: object, working_result: object) -> None:
        self._set_missing_files(missing_files)
    ...
    def init_ui(self):
        ...
        self.tree.setHeaderLabels(["Status", "Name", "", "AES Number", "Type", "Rev", "Status", "integrity"])
        ...
        # Dedicated search-results tree — search populates this; self.tree is
        # never cleared, so exiting search is just a stack page switch.
        self._search_tree = QTreeWidget()
        self._search_tree.setHeaderLabels(["Status", "Name", "", "AES Number", "Type", "Rev", "Status", "integrity"])
        self._search_tree.setColumnWidth(0, 80)
        self._search_tree.setColumnWidth(1, 260)
        self._search_tree.setColumnWidth(2, 70)
        self._search_tree.setColumnWidth(3, 140)
        self._search_tree.setColumnWidth(4, 70)
        self._search_tree.setColumnWidth(5, 50)
        self._search_tree.setColumnWidth(6, 90)
        self._search_tree.setColumnWidth(7, 40)
        ...
        """)
        self._search_tree.setStyleSheet(self.tree.styleSheet())
        ...
        self.release_version_btn.setEnabled(self.perm.can("release_files"))
    ...
    def show_tree_context_menu(self, position):
        ...
        item_id = self._part_id_from_item(item)
        menu = QMenu()
    ...
    def _set_files_tab_enabled(self, enabled: bool):
        ...
        if getattr(self, "release_version_btn", None) is not None:
            self.release_version_btn.setEnabled(bool(enabled) and self.perm.can("release_files"))
    ...
    def add_pdf_to_part(self, part_id: int):
        ...
        try:
            ...
            self._refresh_part_diagnostics(part_id, details)
            self._apply_local_part_updated(part_id, details)
    ...
    def clear_pdf_for_selected(self):
        ...
        try:
            ...
            details = self.bom_service.get_part_details(self.current_part_id) or {}
            self._refresh_part_diagnostics(self.current_part_id, details)
            self._apply_local_part_updated(self.current_part_id, details)
            self.display_details(self.current_part_id)
        except Exception as e:
    ...
    def add_child(self, preselected_parent_id=None):
        ...
        if preselected_parent_id is not None:
            try:
                ...
                pass
            parent_item = self._first_tree_item_by_part_id(preselected_parent_id)
            if parent_item is not None:
                part_type = parent_item.text(4).strip().lower()
                if part_type != "asm":
                    self.window().statusBar().showMessage("Selected parent is not an ASM. Please select a valid assembly.")
                    self._exit_add_child_mode()
                    return
                self.parent_item = parent_item
                self.tree.setCurrentItem(parent_item)
                self.window().statusBar().showMessage(f"Parent selected: {parent_item.text(1)} | Now select the child part...")
    ...
    def _on_tree_click_for_add_child(self, item):
        ...
        if self.parent_item is None:
            part_type = item.text(4).strip().lower()
            if part_type != "asm":
                self.window().statusBar().showMessage("⚠️ Selected part is not an ASM. Please select a valid assembly.")
                return  # Don’t continue

            self.parent_item = item
            self.window().statusBar().showMessage(f"Parent selected: {item.text(1)} | Now select the child part...")
            return
        ...
        if self.child_item is None:
            ...
            parent_name = self.parent_item.text(1)
            child_name = self.child_item.text(1)
            ...
            if confirm == QMessageBox.Yes:
                parent_id = self._part_id_from_item(self.parent_item)
                child_id = self._part_id_from_item(self.child_item)
                try:
                    relation_id = self.bom_service.add_child_by_id(parent_id, child_id)
                    if relation_id == -1:
                        raise ValueError("Parent or child part was not found.")
                    self.window().statusBar().showMessage("Child added successfully.")
                    if not self._apply_local_child_relation_added(parent_id, child_id):
                        self.load_tree()
                except Exception as e:
    ...
    def add_dwg_to_part(self, parent_aes):
        ...
        try:
            ...
            details = self.bom_service.get_part_details(parent_aes) or {}
            if details:
                self._refresh_part_diagnostics(parent_aes, details)
                self._apply_local_part_updated(parent_aes, details)
    ...
    def _exit_search_mode(self) -> None:
        """Switch the stack back to the never-cleared full tree. Zero data touched."""
    ...
    def _perform_search_now(self):
        ...
        if not query:
            # Full tree was never cleared — just flip the stack page back.
            self._exit_search_mode()
            return

        self._in_search_mode = True  # search results land in _search_tree (index 2)
    ...
    def _on_search_failed(self, seq: int, err: str) -> None:
        if int(seq) != int(getattr(self, "_tree_load_seq", 0)):
            return
        try:
            # Clear only the search-results tree; full tree is untouched.
    ...
    def _begin_search_build(self, seq: int, results: list) -> None:
        ...
        # Clear only the search-results tree; self.tree (full tree) is never touched.
        try:
            self._search_tree.clear()
        except Exception:
            pass
    ...
    def _search_build_step(self) -> None:
        ...
        try:
            while self._search_build_queue and processed < 200 and (time.perf_counter() - start) < 0.01:
                ...
                item = self._make_tree_item(part, getattr(self, "_cached_missing_map", None))
    ...
    def _coerce_part_id(self, value):
        try:
    ...
    def _part_id_from_item(self, item):
        if item is None:
            return None
        for column in (1, 0):
            try:
                part_id = self._coerce_part_id(item.data(column, Qt.UserRole))
    ...
    def _missing_issues_for_part(self, part_id, missing_map=None):
        part_id = self._coerce_part_id(part_id)
        if part_id is None:
            return set()

        source = missing_map
        if source is None:
            source = getattr(self, "_cached_missing_map", None) or getattr(self, "_tree_build_missing_map", None) or {}
        try:
            issues = source.get(part_id, set())
            return set(issues or set())
        except Exception:
            pass
        ...
        try:
            for bom_id, issue_type, _filename in (self.missing_files or []):
                if int(bom_id) == part_id:
                    issues.add(str(issue_type))
        except Exception:
            pass
        return issues

    def _set_missing_files(self, missing_files):
        try:
        ...
        missing_ids = set()
        missing_map = {}
        for row in list(self.missing_files or []):
            try:
                bom_id, issue_type, _filename = row
                bom_id = int(bom_id)
                missing_ids.add(bom_id)
                missing_map.setdefault(bom_id, set()).add(str(issue_type))
            except Exception:
                continue

        self.missing_ids = missing_ids
        self._cached_missing_map = missing_map
        self._tree_build_missing_map = missing_map

    def _set_part_diagnostic_rows(self, part_id, rows):
        part_id = self._coerce_part_id(part_id)
        if part_id is None:
            return
        ...
        for row in list(rows or []):
            try:
                kept.append((int(row[0]), str(row[1]), str(row[2] or "")))
            except Exception:
                continue

        self._set_missing_files(kept)

    def _diagnostic_path_exists(self, value: str) -> bool:
        value = str(value or "").strip()
        if not value:
            return True
        path = value if os.path.isabs(value) else os.path.join(str(self.working_dir or ""), value)
        return os.path.exists(path)

    def _diagnostic_rows_for_part(self, part_id, details: dict):
        part_id = self._coerce_part_id(part_id)
        if part_id is None:
            return []
        if not self.working_dir or not os.path.isdir(self.working_dir):
            return [(part_id, "refresh_required", "Run full refresh to re-check diagnostics")]

        details = dict(details or {})
        rows = []
        files_in_dir = os.listdir(self.working_dir)

        filename = str(details.get("filename") or "").strip()
        if filename:
            filename_name = os.path.basename(filename)
            filename_path = filename if os.path.isabs(filename) else os.path.join(self.working_dir, filename_name)
            if not os.path.exists(filename_path):
                rows.append((part_id, "missing_file", filename_name))
            else:
                base_file_name = os.path.splitext(filename_name)[0]
                ...
                if matching_files:
                    ...
                    if filename_name.split(".")[-1] < last_version:
                        rows.append((part_id, "outdated_file", filename_name))

        drawing = str(details.get("drawing") or "").strip()
        if drawing:
            drawing_name = os.path.basename(drawing)
            drawing_path = drawing if os.path.isabs(drawing) else os.path.join(self.working_dir, drawing_name)
            if not os.path.exists(drawing_path):
                rows.append((part_id, "missing_drawing", drawing_name))

        pdf_path = str(details.get("pdf_path") or "").strip()
        if pdf_path and not self._diagnostic_path_exists(pdf_path):
            rows.append((part_id, "missing_pdf", pdf_path))

        step_path = str(details.get("step_path") or "").strip()
        if step_path and not self._diagnostic_path_exists(step_path):
            rows.append((part_id, "missing_step", step_path))

        return rows

    def _refresh_part_diagnostics(self, part_id, details: dict | None = None) -> bool:
        part_id = self._coerce_part_id(part_id)
        if part_id is None:
            return False
        ...
        try:
            if not details:
                details = dict(self.bom_service.get_part_details(part_id) or {})
            details["id"] = part_id
            rows = self._diagnostic_rows_for_part(part_id, details)
        except Exception:
            rows = [(part_id, "refresh_required", "Run full refresh to re-check diagnostics")]

        self._set_part_diagnostic_rows(part_id, rows)

        try:
            for item in self._find_tree_items_by_part_id(part_id, include_search=True):
                self._apply_tree_item_values(item, details, getattr(self, "_cached_missing_map", None))
        except Exception:
            pass

        return not any(str(row[1]) == "refresh_required" for row in rows)

    def _clear_part_diagnostics(self, part_id):
        part_id = self._coerce_part_id(part_id)
        if part_id is not None:
            self._set_part_diagnostic_rows(part_id, [])

    def _apply_tree_item_values(self, item, info: dict, missing_map=None):
        if item is None:
            return
        info = dict(info or {})
        part_id = self._coerce_part_id(info.get("id"))
        ...
        warning_txt = ""
        try:
            if part_id in (self.missing_ids or set()):
                warning_txt = "\u26a0\ufe0f "
        except Exception:
            warning_txt = ""
        ...
        try:
            item.setIcon(0, self._pick_indicator_icon(self._missing_issues_for_part(part_id, missing_map), part_id=part_id))
        except Exception:
            pass
        ...
        if status == "released":
            item.setForeground(6, QColor(0, 128, 0))
        elif status == "obsolete":
            item.setForeground(6, QColor(255, 0, 0))

        if info.get("locked"):
            item.setForeground(2, QColor(255, 0, 0))

    def _make_tree_item(self, info: dict, missing_map=None):
        item = QTreeWidgetItem(["", "", "", "", "", "", "", ""])
        self._apply_tree_item_values(item, info, missing_map)
        return item

    def _build_tree_item_recursive(self, info: dict, missing_map=None):
        node = dict(info or {})
        item = self._make_tree_item(node, missing_map)
        for child in list(node.get("children") or []):
            item.addChild(self._build_tree_item_recursive(child, missing_map))
        return item

    def _iter_tree_items(self, tree_widget=None):
        tree_widget = tree_widget or getattr(self, "tree", None)
        if tree_widget is None:
            return
        for index in range(tree_widget.topLevelItemCount()):
            root = tree_widget.topLevelItem(index)
            stack = [root]
            while stack:
                item = stack.pop()
                yield item
    ...
    def _find_tree_items_by_part_id(self, part_id, include_search: bool = False):
        part_id = self._coerce_part_id(part_id)
        if part_id is None:
            return []

        trees = [getattr(self, "tree", None)]
        if include_search:
            trees.append(getattr(self, "_search_tree", None))

        matches = []
        for tree_widget in trees:
            if tree_widget is None:
                continue
            for item in self._iter_tree_items(tree_widget):
                if self._part_id_from_item(item) == part_id:
                    matches.append(item)
        return matches

    def _first_tree_item_by_part_id(self, part_id):
        matches = self._find_tree_items_by_part_id(part_id, include_search=False)
        return matches[0] if matches else None

    def _direct_child_exists(self, item, child_id):
        child_id = self._coerce_part_id(child_id)
        if item is None or child_id is None:
            return False
        for index in range(item.childCount()):
            if self._part_id_from_item(item.child(index)) == child_id:
                return True
        return False

    def _cached_parent_counts(self):
        tree_data = getattr(self, "_cached_tree_data", None) or {}
        edges = set()

        def visit(node):
            parent_id = self._coerce_part_id((node or {}).get("id"))
            for child in list((node or {}).get("children") or []):
                child_id = self._coerce_part_id((child or {}).get("id"))
                if parent_id is not None and child_id is not None:
                    edges.add((parent_id, child_id))
                visit(child)

        try:
            for root in list(tree_data.values()):
                visit(root)
        except Exception:
            return Counter()
        return Counter(child_id for _parent_id, child_id in edges)

    def _find_cached_node_template(self, part_id):
        part_id = self._coerce_part_id(part_id)
        if part_id is None:
            return None

        tree_data = getattr(self, "_cached_tree_data", None) or {}
        stack = list(tree_data.values())
        while stack:
            node = stack.pop()
            if self._coerce_part_id((node or {}).get("id")) == part_id:
                return copy.deepcopy(node)
            stack.extend(list((node or {}).get("children") or []))
    ...
    def _update_cached_part(self, part_id, part_data: dict):
        part_id = self._coerce_part_id(part_id)
        tree_data = getattr(self, "_cached_tree_data", None)
        if part_id is None or not isinstance(tree_data, dict):
            return False

        changed = False
        new_data = dict(part_data or {})
        new_data["id"] = part_id
    ...
    def _add_cached_root_part(self, part_data: dict):
        part_id = self._coerce_part_id((part_data or {}).get("id"))
        if part_id is None:
            return False

        if not isinstance(getattr(self, "_cached_tree_data", None), dict):
            self._cached_tree_data = {}
        node = dict(part_data or {})
        node["id"] = part_id
        node.setdefault("children", [])
        self._cached_tree_data[part_id] = node
        self._full_tree_cached = True
        return True

    def _add_cached_child_relation(self, parent_id, child_id):
        parent_id = self._coerce_part_id(parent_id)
        child_id = self._coerce_part_id(child_id)
        tree_data = getattr(self, "_cached_tree_data", None)
        if parent_id is None or child_id is None or not isinstance(tree_data, dict):
            return False

        template = self._find_cached_node_template(child_id)
        if not template:
            return False

        parent_counts = self._cached_parent_counts()
        changed = False

        def add_to_parent(node):
            nonlocal changed
            children = (node or {}).setdefault("children", [])
            if self._coerce_part_id((node or {}).get("id")) == parent_id:
                if not any(self._coerce_part_id((child or {}).get("id")) == child_id for child in children):
        ...
        if changed and parent_counts.get(child_id, 0) == 0 and child_id != parent_id:
            try:
                tree_data.pop(child_id, None)
            except Exception:
                pass
        return changed

    def _remove_cached_part(self, part_id):
        part_id = self._coerce_part_id(part_id)
        tree_data = getattr(self, "_cached_tree_data", None)
        if part_id is None or not isinstance(tree_data, dict):
            return None
        ...
        def collect_hoist_children(node):
            for child in list((node or {}).get("children") or []):
                child_id = self._coerce_part_id((child or {}).get("id"))
                if child_id is not None and child_id != part_id and parent_counts.get(child_id, 0) <= 1:
                    hoist_nodes.setdefault(child_id, child)

        def remove_from_children(node):
            nonlocal changed
            kept = []
            for child in list((node or {}).get("children") or []):
                if self._coerce_part_id((child or {}).get("id")) == part_id:
                    collect_hoist_children(child)
                    changed = True
                    continue
        ...
        for root_key, root in list(tree_data.items()):
            if self._coerce_part_id((root or {}).get("id")) == part_id:
                collect_hoist_children(root)
                tree_data.pop(root_key, None)
                changed = True
            else:
                remove_from_children(root)

        if changed:
            for child_id, child_node in hoist_nodes.items():
                if child_id not in tree_data:
                    tree_data[child_id] = child_node
        return hoist_nodes if changed else {}

    def _item_is_attached_to_tree(self, tree_widget, item):
        ...
        while root.parent() is not None:
            root = root.parent()
        for index in range(tree_widget.topLevelItemCount()):
            if tree_widget.topLevelItem(index) is root:
                return True
        return False

    def _remove_top_level_items_by_part_id(self, tree_widget, part_id):
        part_id = self._coerce_part_id(part_id)
        if tree_widget is None or part_id is None:
            return 0
        removed = 0
    ...
    def _delete_part_from_tree_widget(self, tree_widget, part_id, hoist_ids=None):
        part_id = self._coerce_part_id(part_id)
        if tree_widget is None or part_id is None:
            return 0
        hoist_ids = {self._coerce_part_id(x) for x in (hoist_ids or set())}
        hoist_ids.discard(None)
        hoisted = set()
        removed = 0

        targets = [item for item in self._iter_tree_items(tree_widget) if self._part_id_from_item(item) == part_id]
        for item in reversed(targets):
            if not self._item_is_attached_to_tree(tree_widget, item):
                continue

            for child_index in range(item.childCount() - 1, -1, -1):
                child = item.child(child_index)
                child_id = self._part_id_from_item(child)
                if child_id in hoist_ids and child_id not in hoisted:
                    moved = item.takeChild(child_index)
                    tree_widget.addTopLevelItem(moved)
                    hoisted.add(child_id)
    ...
    def _refresh_search_after_local_mutation(self):
        try:
            if self.search_input.text().strip() and getattr(self, "_in_search_mode", False):
                self._perform_search_now()
        except Exception:
            pass

    def _apply_local_part_added(self, part_id, part_data: dict):
        part_id = self._coerce_part_id(part_id)
        if part_id is None:
            return False
        node = dict(part_data or {})
        ...
        if not self._add_cached_root_part(node):
            return False

        try:
            item = self._build_tree_item_recursive(node, getattr(self, "_cached_missing_map", None))
        ...
        self._refresh_search_after_local_mutation()
        return True

    def _apply_local_part_updated(self, part_id, part_data: dict):
        part_id = self._coerce_part_id(part_id)
        if part_id is None:
            return False
        node = dict(part_data or {})
        ...
        cache_changed = self._update_cached_part(part_id, node)
        items = self._find_tree_items_by_part_id(part_id, include_search=False)
        for item in items:
            self._apply_tree_item_values(item, node, getattr(self, "_cached_missing_map", None))

        self._refresh_search_after_local_mutation()
        return cache_changed or bool(items)

    def _apply_local_part_deleted(self, part_id):
        part_id = self._coerce_part_id(part_id)
        if part_id is None:
            return False

        hoist_nodes = self._remove_cached_part(part_id)
        if hoist_nodes is None:
            return False

        removed = self._delete_part_from_tree_widget(self.tree, part_id, set(hoist_nodes.keys()))
        self._delete_part_from_tree_widget(getattr(self, "_search_tree", None), part_id, set())
        self._last_tree_node_count = max(0, int(self._last_tree_node_count or 0) - max(1, removed))
        self._refresh_search_after_local_mutation()
        return True

    def _apply_local_child_relation_added(self, parent_id, child_id):
        parent_id = self._coerce_part_id(parent_id)
        child_id = self._coerce_part_id(child_id)
        ...
        parent_counts = self._cached_parent_counts()
        child_template = self._find_cached_node_template(child_id)
        if not child_template:
            return False

        cache_changed = self._add_cached_child_relation(parent_id, child_id)
        parent_items = self._find_tree_items_by_part_id(parent_id, include_search=False)
        changed_tree = False
        for parent_item in parent_items:
            if self._direct_child_exists(parent_item, child_id):
                continue
            parent_item.addChild(self._build_tree_item_recursive(copy.deepcopy(child_template), getattr(self, "_cached_missing_map", None)))
            parent_item.setExpanded(True)
            changed_tree = True

        if (cache_changed or changed_tree) and parent_counts.get(child_id, 0) == 0 and child_id != parent_id:
            self._remove_top_level_items_by_part_id(self.tree, child_id)

        self._refresh_search_after_local_mutation()
        return cache_changed or changed_tree
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            try:
                ...
                if type(added) == int:
                    ...
                    details = self.bom_service.get_part_details(added) or dict(part_data)
                    details["id"] = added
                    self._refresh_part_diagnostics(added, details)
                    if not self._apply_local_part_added(added, details):
                        self.load_tree()
                    try:
                        self.display_details(added)
                    except Exception:
                        pass
                elif added == "existing":
    ...
    def edit_part(self, id=None):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            try:
                ...
                details = self.bom_service.get_part_details(id) or dict(updated_data)
                details["id"] = id
                self._refresh_part_diagnostics(id, details)
                if not self._apply_local_part_updated(id, details):
                    self.load_tree()
    ...
    def delete_part(self, id=None):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                self._clear_part_diagnostics(id)
                if not self._apply_local_part_deleted(id):
                    self.load_tree()
                self.clear_details()
            except Exception as e:
    ...
    def _refresh_current_tree_item_lock_state(self, part_id: int | None = None):
        ...
        Updates the cached node and every visible row for this part.
        """
        try:
            current_item = self.tree.currentItem()
            pid = part_id or self._part_id_from_item(current_item)
            if not pid:
                return

            info = self.bom_service.get_part_details(int(pid)) or {}
            self._update_cached_part(int(pid), info)
            for item in self._find_tree_items_by_part_id(int(pid), include_search=True):
                self._apply_tree_item_values(item, info, getattr(self, "_cached_missing_map", None))
        except Exception:
            pass
    ...
    def checkout_part(self, part_id=None):
        ...
        if reply == QMessageBox.Yes:
            try:
                self.bom_service.checkout_part(part_id)
    ...
    def _tree_build_step(self) -> None:
        ...
        try:
            while self._tree_build_queue and processed < 50 and (time.perf_counter() - start) < 0.01:
                ...
                item = self._make_tree_item(info, missing_map)
            ...
            if not self._tree_build_queue:
                ...
                # self.tree is never cleared after a search — no clone cache needed.
    ...
    def _restore_cached_full_tree(self):
        """Kept for backward compatibility. Full tree lives in self.tree permanently."""
        self._exit_search_mode()
    ...
    def display_details(self, item_id):
        ...
        if self.missing_files:
            ...
            for bom_id, issue_type, filename in self.missing_files:
                if bom_id == item_id:
                    if issue_type == 'refresh_required':
                        missing_msgs.append(f"Refresh required: {filename}")
    ...
    def _make_indicator_icon(self, pdf_ok: bool, step_ok: bool, step_present: bool = True) -> QIcon:
        """Create indicator badges using emoji icons (like _EVENT_STYLES) for PDF and STEP status."""
        badge_size = 64
        gap = 4
        total_width = badge_size * 2 + gap if step_present else badge_size
        pm = QPixmap(total_width, badge_size)
        pm.fill(Qt.transparent)

        painter = QPainter(pm)
        painter.setRenderHint(QPainter.Antialiasing, True)

        def draw_badge(x: int, icon: str, ok: bool):
            """Draw a colored badge with emoji icon (green when OK, red when not)."""
            # Match _EVENT_STYLES color scheme
            if ok:
                bg_color = QColor(255, 255, 255)    # green-600 (released/ok)
                border_color = QColor(16, 185, 129)  # green-500
            else:
                bg_color = QColor(255, 255, 255)    # red-500 (missing/outdated)
                border_color = QColor(220, 38, 38)   # red-600

            # Draw rounded rectangle background
            painter.setPen(QPen(border_color, 2))
            painter.setBrush(bg_color)
            painter.drawRoundedRect(x, 1, badge_size - 2, badge_size - 2, 4, 4)
            ...
            painter.drawText(x, 0, badge_size - 2, badge_size, Qt.AlignCenter, icon)
        ...
        # STEP badge
        if step_present:
            draw_badge(badge_size + gap, "📐", step_ok)
    ...
    def _refresh_current_tree_item_indicator(self):
        ...
        try:
            ...
            if not item:
                return
            part_id = self._part_id_from_item(item)
            if not part_id:
                return

            issues = self._missing_issues_for_part(part_id)
            for matching_item in self._find_tree_items_by_part_id(part_id, include_search=True):
                matching_item.setIcon(0, self._pick_indicator_icon(issues, part_id=int(part_id)))
